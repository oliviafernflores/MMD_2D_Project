...
#SBATCH --job-name="GER_HEL_bottlegrowth_split_inbreeding"
...
def bottlegrowth_split_demography(fs, ns, pts):
    ...
    upper = [10, 10, 1, 3, 1, 1]
    try:
        fid = open(f'demo_results/GER_HEL_bottlegrowth_split_inbreeding_pop1_only_demo_fits{process_ii}.txt', 'a')
    except:
        fid = open(f'demo_results/GER_HEL_bottlegrowth_split_inbreeding_pop1_only_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
        p0 = dadi.Misc.perturb_params(params, fold = 0, upper_bound = upper, lower_bound = lower)
        print('Beginning bottlegrowth_split_inbreeding_pop1_only optimization ' + str(i) + '*'*20)
        ...
        print('Finished bottlegrowth_split_inbreeding_pop1_only optimization ' + str(i) + '*'*20)
    fid.close()
def main():
