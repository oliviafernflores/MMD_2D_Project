...
#SBATCH --job-name="GER_HEL_split_delay_mig_inbreeding"
...
def split_delay_mig_demography(fs, ns, pts):
    ...
    lower = [1e-2, 1e-2, 1e-4, 1e-3, 1e-3, 1e-3, 1e-3, 0]
    upper = [10, 10, 1, 1, 10, 1, 1, 1]
    try:
        fid = open(f'demo_results/GER_HEL_split_delay_mig_inbreeding_pop1_only_demo_fits{process_ii}.txt', 'a')
    except:
        fid = open(f'demo_results/GER_HEL_split_delay_mig_inbreeding_pop1_only_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
        p0 = dadi.Misc.perturb_params(params, fold = 0, upper_bound = upper, lower_bound = lower)
        print('Beginning split_delay_mig_inbreeding_pop1_only optimization ' + str(i) + '*'*20)
        ...
        print('Finished split_delay_mig_inbreeding_pop1_only optimization ' + str(i) + '*'*20)
    fid.close()
def main():
