...
def split_mig_demography(fs, ns, pts):
    ...
    upper = [3, 6, 1, 3, 1, 1]
    try:
        fid = open(f'demo_results/GER_HEL_split_mig_inbreeding_pop1_only_demo_fits{process_ii}.txt', 'a')
    except:
        fid = open(f'demo_results/GER_HEL_split_mig_inbreeding_pop1_only_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
        p0 = dadi.Misc.perturb_params(params, fold = 0, upper_bound = upper, lower_bound = lower)
        print('Beginning split_mig_inbreeding_pop1_only optimization ' + str(i) + '*'*20)
        ...
        print('Finished split_mig_inbreeding_pop1_only optimization ' + str(i) + '*'*20)
    fid.close()
def main():
