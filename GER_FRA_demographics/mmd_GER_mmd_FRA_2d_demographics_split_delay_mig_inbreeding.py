...
def split_delay_mig_demography(fs, ns, pts):
    ...
    lower = [1e-2, 1e-2, 1e-4, 1e-3, 1e-3, 1e-3, 1e-10, 1e-10, 0]
    upper = [10, 10, 1, 1, 10, 1, 1, 1, 1]
    try:
        fid = open(f'demo_results/FRA_GER_split_delay_mig_inbreeding_demo_fits{process_ii}.txt', 'a')
    except:
        fid = open(f'demo_results/FRA_GER_split_delay_mig_inbreeding_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
