...
def split_asym_mig_demography(fs, ns, pts):
    ...
    lower = [1e-2, 1e-2, 1e-3, 1e-3, 1e-3, 1e-10, 1e-5, 0]
    upper = [3, 6, 1, 10, 10, 1, 1, 1]
    try:
        fid = open(f'demo_results/FRA_GER_split_asym_mig_inbreeding_demo_fits{process_ii}.txt', 'a')
    except:
        fid = open(f'demo_results/FRA_GER_split_asym_mig_inbreeding_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
