...
def plot_im_demography(fs, ns, pts, popt):
    ...
    fig = plt.figure('FRA_GER_IM_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_im_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_im_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_im_demography_demesdraw.png')
def plot_im_pre_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_IM_pre_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_im_pre_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_im_pre_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_im_pre_demography_demesdraw.png')
def plot_bottlegrowth_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_Bottlegrowth_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_bottlegrowth_demography.png')
    # out = dadi.Demes.output()
    # demes.dump(out, 'demo_results/FRA_GER_bottlegrowth_demo_fits_demes.yaml')
    # ax = demesdraw.tubes(out)
    # ax.figure.savefig('plots/FRA_GER_bottlegrowth_demography_demesdraw.png')
def plot_bottlegrowth_split_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_Bottlegrowth_Split_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_bottlegrowth_split_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_bottlegrowth_split_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_bottlegrowth_split_demography_demesdraw.png')
def plot_bottlegrowth_split_mig_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_Bottlegrowth_Split_Migration_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_bottlegrowth_split_mig_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_bottlegrowth_split_mig_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_bottlegrowth_split_mig_demography_demesdraw.png')
def plot_split_asym_mig_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_Split_Asymmetric_Migration_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_split_asym_mig_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_split_asym_mig_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_split_asym_mig_demography_demesdraw.png')
def plot_split_delay_mig_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_Split_Delay_Migration_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_split_delay_mig_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_split_delay_mig_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_split_delay_mig_demography_demesdraw.png')
def plot_split_mig_demography(fs, ns, pts, popt):
    ...
    demo_model_ex = dadi.Numerics.make_extrap_func(demo_model)
    model = demo_model_ex(popt, ns, pts)
    fig = plt.figure('FRA_GER_Split_Migration_Demography')
    fig.clear()
    dadi.Plotting.plot_2d_comp_multinom(model, fs)
    fig.savefig('plots/FRA_GER_split_mig_demography.png')
    out = dadi.Demes.output(deme_mapping = {'ancestral':['d1_1'], 'GERMANY': ['d1_2'], 'FRANCE': ['d2_2']})
    demes.dump(out, 'demo_results/FRA_GER_split_mig_demo_fits_demes.yaml')
    ax = demesdraw.tubes(out)
    ax.figure.savefig('plots/FRA_GER_split_mig_demography_demesdraw.png')
def main():
    ...
    #im_demography
    im_popt = get_popt('demo_results/FRA_GER_im_demo_fits_combined.txt')
    plot_im_demography(fs, ns, pts_l, im_popt)

    #im_pre_demography
    im_pre_popt = get_popt('demo_results/FRA_GER_im_pre_demo_fits_combined.txt')
    ...
    split_asym_mig_popt = get_popt('demo_results/FRA_GER_split_asym_mig_demo_fits_combined.txt')
    plot_split_asym_mig_demography(fs, ns, pts_l, split_asym_mig_popt)

    #split_delay_mig_demography
    split_delay_mig_popt = get_popt('demo_results/FRA_GER_split_delay_mig_demo_fits_combined.txt')
    plot_split_delay_mig_demography(fs, ns, pts_l, split_delay_mig_popt)

    #split_mig_demography
    split_mig_popt = get_popt('demo_results/FRA_GER_split_mig_demo_fits_combined.txt')
