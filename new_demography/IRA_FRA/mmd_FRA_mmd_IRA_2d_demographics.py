...
def snm_demography(fs, ns, pts):
    ...
    dadi.Plotting.plot_2d_comp_multinom(model, fs, vmin = 1, resid_range = 3, pop_ids = ['Mmd_FRA', 'Mmd_IRA'], show = False)
...
def im_pre_demography(fs, ns, pts):
    ...
    try:
        ...
        fid = open(f'demo_results/IRA_FRA_im_pre_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
        p0 = dadi.Misc.perturb_params(params, fold = 1, upper_bound = upper, lower_bound = lower)
...
def bottlegrowth_demography(fs,  ns, pts):
    ...
    try:
        ...
        fid = open(f'demo_results/IRA_FRA_bottlegrowth_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
        p0 = dadi.Misc.perturb_params(params, fold = 1, upper_bound = upper, lower_bound = lower)
...
def bottlegrowth_split_mig_demography(fs, ns, pts):
    ...
    try:
        ...
        fid = open(f'demo_results/IRA_FRA_bottlegrwoth_split_mig_demo_fits{process_ii}.txt', 'w')
    for i in range(20):
        p0 = dadi.Misc.perturb_params(params, fold = 1, upper_bound = upper, lower_bound = lower)
