...
# Read the input text file using read.delim
go_terms <- read.delim("/Users/olivia/Documents/2D_demographics_DFE/MMD_2D_Project/gene_ontology/direct_biological_process_terms.txt", 
                       header = TRUE, 
                       stringsAsFactors = FALSE)

# Function to get all descendants of a given GO term using iterative approach
get_all_descendants <- function(goid) {
  all_goids <- keys(GO.db::GO.db, keytype = "GOID")
  descendants <- c()
  queue <- c(goid)  # Initialize the queue with the starting GO term
...
for (i in 1:nrow(go_terms)) {
  ...
  output_file <- paste0("/Users/olivia/Documents/2D_demographics_DFE/MMD_2D_Project/gene_ontology/all_descendants_of_", gsub(":", "_", goid), ".txt")
